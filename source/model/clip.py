...
class MappingNetwork(nn.Module):
    def __init__(self,
        ...
        for idx, input_dim, output_dim in features:
            ...
            layer = FullyConnectedLayer(input_dim, output_dim, activation=activation, lr_multiplier=self.lr_multiplier)
            setattr(self, 'linear_{}'.format(idx), layer)
        self.register_buffer('w_avg', torch.zeros([output_dim]))
        self.bn = nn.BatchNorm1d(self.input_dim) if batch_norm else None
...
class Projection(nn.Module):
    def __init__(self, feature_dim, hidden_dim, embed_dim, batch_norm, final_Activation):
        super().__init__()
        ...
        self.RELU = nn.ReLU()
        self.bn = nn.BatchNorm1d(feature_dim) if batch_norm else None
...
class ClipModel(nn.Module):
    def __init__(self,
        ...
        if proj_type == 'projection':
            ...
            self.audio_projection = Projection(527, hidden_dim, embed_dim, batch_norm, final_activation)
            self.video_projection = Projection(vision_embed_dim, hidden_dim, embed_dim, batch_norm, final_activation) if video_projection else None
            self.init()
        elif proj_type == 'mapping':
            self.audio_projection = MappingNetwork(audio_label_dim, embed_dim, final_activation, hidden_dim, batch_norm, num_layers=num_layers)
            self.video_projection = MappingNetwork(vision_embed_dim, embed_dim, final_activation, hidden_dim, batch_norm, num_layers=num_layers) if video_projection else None
