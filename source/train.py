...
def encode_and_save_embeddings(model, dataloader, savepath):
    ...
    with torch.no_grad():
        for num_batch in tqdm(range(len(dataloader)), desc="Encoding and saving train embeddings: "):
            video_data, audio_data = dataloader.load_data(num_batch)
            video_data = torch.from_numpy(video_data).to(torch.float32).to(device)
            audio_data = torch.from_numpy(audio_data).to(torch.float32)

            features = torch.stack([make_features(feature.unsqueeze(0), dataloader.sr) for feature in audio_data])
            features = features.to(device)

            audio_encs = model.encode_audio(features)
            video_encs = model.encode_video(video_data)

            embeddings_audio.append(audio_encs.cpu().numpy())
            embeddings_video.append(video_encs.cpu().numpy())

    embeddings_audio = np.concatenate(embeddings_audio, axis=0)
    embeddings_video = np.concatenate(embeddings_video, axis=0)

    np.savez(os.path.join(savepath, 'train_embeddings'), audio=embeddings_audio, video=embeddings_video)

    embeddings_audio = []
    embeddings_video = []

    with torch.no_grad():
        for num_batch in tqdm(range(dataloader.len_test_batches()), desc="Encoding and saving test embeddings: "):
            video_data, audio_data = dataloader.load_data(dataloader.get_test_batch(num_batch))
            video_data = torch.from_numpy(video_data).to(torch.float32).to(device)
            audio_data = torch.from_numpy(audio_data).to(torch.float32)
    ...
    np.savez(os.path.join(savepath, 'test_embeddings'), audio=embeddings_audio, video=embeddings_video)
